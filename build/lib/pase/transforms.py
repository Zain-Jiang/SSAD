...
from pase.ASV_FeatureExtraction.ASVspoof_API import extract_cqcc,extract_lfcc,fft_pre
from pase.ASV_FeatureExtraction.logpowerspec import logpowspec
try:
...
class LPS(object):
    ...
    def __call__(self, pkg, cached_file=None):
        ...
        # print(logpowspec(np.array(wav)).shape)
        LPS=logpowspec(np.array(wav))[:200,:].T
        #print ('Chunks wav shape is {}'.format(wav.shape))
        X=torch.from_numpy(LPS).float()
        pkg[self.name] = X
...
class CQCC(object):
    ...
    def __call__(self, pkg, cached_file=None):
        ...
        fs=16000
        CQCC=extract_cqcc(np.array(wav), fs, 96, fs/2, fs/2**10, 16, 29, 'ZsdD')
        CQCC=CQCC[:,0:200]
        X=torch.from_numpy(CQCC).float()
        pkg[self.name] = X
...
class LFCC(object):
    ...
    def __call__(self, pkg, cached_file=None):
        ...
        fs=16000
        freqs, raw_fft = fft_pre(np.array(wav),fs,0.025,0.0095,512)
        stat,delta,double_delta = extract_lfcc(raw_fft,fs,512,20)
        cell = np.concatenate((stat,delta,double_delta),axis=1).T
        cell = cell[:,0:200]
        X=torch.from_numpy(cell).float()
        pkg[self.name] = X
