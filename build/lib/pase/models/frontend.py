...
try:
    ...
    from .aspp import aspp_resblock
    from .tdnn import TDNN
from pase.models.tcn import TemporalConvNet as TCN
    ...
    def __init__(self, num_inputs=1, 
        ...
        if rnn_pool:
            if rnn_dim is None:
                rnn_dim = emb_dim
            if rnn_type!="tcn":
                self.rnn = build_rnn_block(fmap, rnn_dim ,
                                       rnn_layers=rnn_layers,
                                       rnn_type=rnn_type,
                                       bidirectional=False,
                                       dropout=rnn_dropout)
                ...
                out_channel = [128] * 12
                self.rnn = TCN(fmap, out_channel , kernel_size=9 , dropout=0)
            self.W1 = nn.Conv1d(rnn_dim, 256, 1)
        else:
            self.W1 = nn.Conv1d(rnn_dim, rnn_dim, 1)
            self.W2 = nn.ReLU(inplace=True)
            self.W3 = nn.Conv1d(rnn_dim, emb_dim, 1)
