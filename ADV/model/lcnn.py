...
class LCNN(nn.Module):
    def __init__(self, c_s=[32, 48, 64, 32, 32, 80], asoftmax=True, phiflag=True, num_classes=2): # inputs shape:[863,600,1]
        ...
        self.layer1 = nn.Sequential(
            mfm(1, c_s[0], 5, 1, 2),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=False))  #shape[431,300,c_s[0]] #shape[128,300,c_s[0]]

        self.layer2 = nn.Sequential(
            group(c_s[0], c_s[1], 3, 1, 1),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=False),  #shape[215,150,c_s[1]]#shape[64,150,c_s[0]]
            nn.BatchNorm2d(c_s[1]))

        self.layer3 = nn.Sequential(
            group(c_s[1], c_s[2], 3, 1, 1),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=False))  #shape[107,75,c_s[2]]#shape[32,75,c_s[0]]

        self.layer4 = nn.Sequential(
            group(c_s[2], c_s[3], 3, 1, 1),
            nn.BatchNorm2d(c_s[3]),
            group(c_s[3], c_s[4], 3, 1, 1),
            nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=False))  #shape[53,37,c_s[4]]#shape[16,37,c_s[0]]

        self.fc1 = nn.Sequential(           
            mfm(16*37*c_s[4], c_s[5], type=0, dp_out=0.75),
