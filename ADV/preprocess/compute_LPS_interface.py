...
def build_from_path(wavlist, out_dir, stft_conf, num_workers=1):
    ...
    f = open('./uploaded_wav/protocol.txt','w',encoding='utf-8')
    for wav_path in wavlist:
        utt_idx = os.path.basename(wav_path).rstrip('.flac')
        f.write("LA_0069 "+utt_idx+" - - bonafide")
        f.write("\n")
    f.close()
...
if __name__ == "__main__":
    ...
    wavfile_list_train = glob.glob(os.path.join(args.in_dir, "*.flac"))
    wavfile_list_train += glob.glob(os.path.join(args.in_dir, "*.wav"))
    wavfile_list_train += glob.glob(os.path.join(args.in_dir, "*.mp3"))
    wavfile_list_train += glob.glob(os.path.join(args.in_dir, "*.m4a"))
