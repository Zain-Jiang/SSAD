...
def load_label(path):
    protocol_file = path + "/ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.train.trn.txt"
    flac_path = path + "/ASVspoof2019_LA_train/flac/"
    ...
    for item in protocol_list:
        item = item.split(" ")
        if item[-1] == "bonafide":
            dic[flac_path + item[0] + ".flac"] = 0
        # elif item[-2][0]=="A":
        # dic[item[0]+".flac"]=int(item[-2][2])
        else:
            dic[flac_path + item[0] + ".flac"] = 1
    return dic
...
class LibriSpeechSegTupleWavDataset(PairWavDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # get label for classification task
        root = args[0].split('/')[1:-3]
        root = "/" + ("/").join(root)
        self.flac_labels = load_label(root)
    ...
    def __getitem__(self, index):
        ...
        if sample_probable(self.zero_speech_p):
            ...
            if len(neighbors) > 0:
                ...
                indices.remove(index)
                while True:
                    cindex = random.choice(indices)
                    cwname = os.path.join(self.data_root, self.wavs[cindex]['filename'])
                    if self.flac_labels[prefix]==self.flac_labels[cwname]:
                        cwav = self.retrieve_cache(cwname, self.wav_cache)
                        break
        ...
        while True:
            rindex = random.choice(indices)
            rwname = os.path.join(self.data_root, self.wavs[rindex]['filename'])
            if self.flac_labels[prefix] != self.flac_labels[rwname]:
                rwav = self.retrieve_cache(rwname, self.wav_cache)
                break
