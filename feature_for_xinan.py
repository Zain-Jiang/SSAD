...
import os
import sys
from ASR.neural_networks import MLP,context_window
import torch
import numpy as np
...
pase_model = "XinAn_ckpt/FE_e799.ckpt"  # e.g, '../FE_e199.ckp' (download the pre-trained PASE+ model as described in the doc)
data_folder = "data/XinAn/wav"  # e.g., '/home/mirco/Dataset/TIMIT'
...
f = open('data/XinAn/result/protocol.txt','w',encoding='utf-8')
...
for item in tqdm(os.listdir(data_folder)):
    ...
    with torch.no_grad():
        ...
        for wi, snt_id in enumerate(fea.keys()):
            ...
            np.save("data/XinAn/result/"+snt_id+".npy",fea_pase[snt_id])
            if snt_id[0]=="R":
